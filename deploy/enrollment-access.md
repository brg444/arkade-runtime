...
Public `/v1/status` reports `enrollmentMode: "open"` or `"token"`. The wallet
follows that response. Changing the service setting does not require a browser
build or change existing vault descriptors.
