...
func vaultRecordFromCredential(c Credential) VaultRecord {
	return VaultRecord{
		...
		ExternalOwnerWallet:  bytes.Clone(c.ExternalOwnerWallet),
		RecoveryKey:          append([]byte(nil), c.RecoveryKey...),
		VaultCosignerBase:    append([]byte(nil), c.VaultCosignerBase...),
		ArkadeCosignerBase:   bytes.Clone(c.ArkadeCosignerBase),
		...
		SavingsAddress: c.SavingsAddress, SavingsScript: bytes.Clone(c.SavingsScript),
