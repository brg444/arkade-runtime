...
func reservePendingEnrollmentTx(tx *sql.Tx, p PendingEnrollment) (*PendingEnrollment, error) {
	...
	if _, err := tx.Exec(`
INSERT INTO pending_enrollment (handle, vault_id, token_hash, challenge, policy_digest, expires_at, created_at)
VALUES (?, ?, ?, ?, ?, ?, ?)`,
		p.Handle, p.VaultID, p.TokenHash, p.Challenge, p.PolicyDigest, p.ExpiresAt, p.CreatedAt,
	); err != nil {
...
func (l *Ledger) GetPendingByHandle(handle string) (*PendingEnrollment, error) {
	...
	err := l.db.QueryRow(`
SELECT handle, vault_id, token_hash, challenge, policy_digest, expires_at, created_at
