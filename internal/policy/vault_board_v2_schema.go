...
const vaultBoardV2SchemaVersion = 2

var vaultBoardV2Tables = []string{
	"vault_board_v2_authorization",
	"vault_board_v2_dispatch",
	"vault_board_v2_enrollment",
	"vault_board_v2_operation",
	"vault_board_v2_submission",
}

var expectedVaultBoardV2Tables = map[string]struct {
	cols    []colSpec
	fks     []fkSpec
	indexes []idxSpec
}{
	...
	"vault_board_v2_operation": {
		...
		fks: []fkSpec{{Table: "vault_board_v2_enrollment", From: "vault_id", To: "vault_id"}},
		indexes: []idxSpec{
			{Name: "", Unique: true, Cols: []string{"operation_id"}},
			{Name: "", Unique: true, Cols: []string{"vault_id", "txid", "vout"}},
			{Name: "vault_board_v2_operation_vault", Unique: false, Cols: []string{"vault_id", "created_at"}},
		},
	},
	"vault_board_v2_authorization": {
		cols: []colSpec{
		...
		fks: []fkSpec{{Table: "vault_board_v2_operation", From: "operation_id", To: "operation_id"}},
		indexes: []idxSpec{
			{Name: "", Unique: true, Cols: []string{"operation_id", "attempt", "phase"}},
			{Name: "vault_board_v2_authorization_phase", Unique: false, Cols: []string{"phase", "created_at"}},
		},
	},
	"vault_board_v2_submission": {
		cols: []colSpec{
		...
		fks: []fkSpec{
			{Table: "vault_board_v2_dispatch", From: "operation_id", To: "operation_id"},
			{Table: "vault_board_v2_dispatch", From: "attempt", To: "attempt"},
			{Table: "vault_board_v2_dispatch", From: "phase", To: "phase"},
			{Table: "vault_board_v2_authorization", From: "operation_id", To: "operation_id"},
			{Table: "vault_board_v2_authorization", From: "attempt", To: "attempt"},
			{Table: "vault_board_v2_authorization", From: "phase", To: "phase"},
		},
...
const createVaultBoardV2Schema = `
CREATE TABLE vault_board_v2_enrollment (
  vault_id TEXT PRIMARY KEY REFERENCES vault(vault_id),
  program TEXT NOT NULL CHECK (program = 'vault-board-v2'),
...
CREATE TABLE vault_board_v2_operation (
  operation_id TEXT PRIMARY KEY,
  vault_id TEXT NOT NULL REFERENCES vault_board_v2_enrollment(vault_id),
...
CREATE TABLE vault_board_v2_authorization (
  operation_id TEXT NOT NULL REFERENCES vault_board_v2_operation(operation_id),
...
CREATE TABLE vault_board_v2_submission (
  ...
  integrity_mac BLOB NOT NULL CHECK (length(integrity_mac) = 32),
  PRIMARY KEY (operation_id, attempt, phase),
  FOREIGN KEY (operation_id, attempt, phase) REFERENCES vault_board_v2_authorization(operation_id, attempt, phase),
	FOREIGN KEY (operation_id, attempt, phase) REFERENCES vault_board_v2_dispatch(operation_id, attempt, phase),
  CHECK (
...
CREATE TABLE vault_board_v2_dispatch (
	operation_id TEXT NOT NULL,
	attempt INTEGER NOT NULL,
	phase TEXT NOT NULL,
	request_digest BLOB NOT NULL CHECK (length(request_digest) = 32),
...
CREATE INDEX vault_board_v2_authorization_phase ON vault_board_v2_authorization(phase, created_at);
CREATE INDEX vault_board_v2_operation_vault ON vault_board_v2_operation(vault_id, created_at);
`

// OpenMutinynetVaultBoardV2Ledger is an explicit fresh-release gate. It does
// not upgrade or reinterpret a v1 database; callers must deliberately choose
// the Mutinynet v2 baseline.
func OpenMutinynetVaultBoardV2Ledger(path string, clock Clock) (*Ledger, error) {
	...
	if err := initializeOrValidateVaultBoardV2Schema(db); err != nil {
		_ = db.Close()
		return nil, err
	}
...
func initializeOrValidateVaultBoardV2Schema(db *sql.DB) error {
	...
	if len(tables) == 0 {
		...
		defer tx.Rollback()
		if _, err := tx.Exec(createMultiTenantSchema + createMainnetVtxoSchema + createVaultBoardV2Schema); err != nil {
			return fmt.Errorf("create vault-board-v2 schema: %w", err)
		}
		if _, err := tx.Exec(`INSERT INTO schema_meta (version) VALUES (?)`, vaultBoardV2SchemaVersion); err != nil {
			return err
		}
		if err := tx.Commit(); err != nil {
			return err
		}
		tables = append(append([]string(nil), mainnetTables...), vaultBoardV2Tables...)
	}
	want := append(append([]string(nil), mainnetTables...), vaultBoardV2Tables...)
	sort.Strings(want)
	if !sameStrings(tables, want) {
		return fmt.Errorf("database is not the Mutinynet vault-board-v2 baseline: tables %v", tables)
	}
	if err := validateVaultBoardV2Objects(db); err != nil {
		return err
	}
	...
	if rows != 1 || ver != vaultBoardV2SchemaVersion {
		return fmt.Errorf("database is not the Mutinynet vault-board-v2 baseline: schema version %d", ver)
	}
	if err := validateMultiTenantSchemaOn(db); err != nil {
		return err
	}
	if err := validateVaultBoardV2Tables(db); err != nil {
		return err
	}
...
func validateVaultBoardV2Tables(db *sql.DB) error {
	for _, table := range vaultBoardV2Tables {
		wantStruct := expectedVaultBoardV2Tables[table]
		...
		if err := matchVaultBoardV2Indexes(table, indexes, wantStruct.indexes); err != nil {
			return err
		}
		var createSQL string
		if err := db.QueryRow(`SELECT sql FROM sqlite_master WHERE type='table' AND name=?`, table).Scan(&createSQL); err != nil {
			return fmt.Errorf("vault-board-v2 table %s: %w", table, err)
		}
		want := extractChecksByTable(createVaultBoardV2Schema)[table]
...
func matchVaultBoardV2Indexes(table string, got, want []idxSpec) error {
	if len(got) != len(want) {
...
func validateVaultBoardV2Objects(db *sql.DB) error {
	want := append([]string(nil), mainnetTables...)
	for i, table := range want {
		want[i] = "table:" + table
	}
	want = append(want,
		"table:vault_board_v2_authorization", "table:vault_board_v2_enrollment",
		"table:vault_board_v2_dispatch", "table:vault_board_v2_operation", "table:vault_board_v2_submission",
		...
		"index:vault_board_v2_authorization_phase", "index:vault_board_v2_operation_vault",
	)
	...
	if !sameStrings(got, want) {
		return fmt.Errorf("database is not the Mutinynet vault-board-v2 baseline: objects %v", got)
	}
	return nil
}
