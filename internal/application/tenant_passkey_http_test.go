...
func TestHTTPTenantBInstallRecoverDoesNotTouchA(t *testing.T) {
	...
	if _, _, err := reopened.IssueForTest(context.Background(), tenantB, bytes.Repeat([]byte{0x77}, 32), 1_000, 50, fixture.PeriodAllowanceSats, func(context.Context) (string, error) {
		return "signed-b", nil
	}); err != nil {
		t.Fatal(err)
	}
