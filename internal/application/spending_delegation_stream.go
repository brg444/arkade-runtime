...
func delegationStreamPhase(e spendingDelegationStreamEvent, batch string) (string, error) {
	if batch == "" {
		return "", nil
	}
...
func replayDelegationStream(ctx context.Context, saved *policy.SpendingDelegationSnapshot, live <-chan spendingDelegationStreamEvent) (<-chan spendingDelegationStreamEvent, error) {
	names := []string{}
	...
	replay := make([]spendingDelegationStreamEvent, 0, len(names))
	for _, name := range names {
		var e spendingDelegationStreamEvent
		if err := json.Unmarshal([]byte(saved.Events[name].Evidence), &e); err != nil {
			return nil, err
		}
		replay = append(replay, e)
	}
	out := make(chan spendingDelegationStreamEvent, 8)
	go func() {
		defer close(out)
